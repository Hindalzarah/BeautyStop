...
class WishlistApiServiceRepository(val context: Context) {
    ...
    suspend fun getWishlist(id: String) =
        retrofitApi.getWishlist(FirebaseAuth.getInstance().currentUser!!.uid)
    ...
    suspend fun editWishlist(id: String, wishlistBody: WishlistModel) =
        retrofitApi.editWishlist(id, wishlistBody)


    //Shopping bag
    suspend fun getShoppingBag(id: String) =
        retrofitApi.getShoppingBag(FirebaseAuth.getInstance().currentUser!!.uid)

    suspend fun addToShoppingCart(shoppingBagBody: ShoppingBagModel) =
        retrofitApi.addToShoppingBag(shoppingBagBody)

    suspend fun deleteFromShoppingBag(id: String) = retrofitApi.deleteFromShoppingBag(id)
    suspend fun editShoppingBag(id: String, shoppingBagBody: ShoppingBagModel) =
        retrofitApi.editShoppingBag(id, shoppingBagBody)
